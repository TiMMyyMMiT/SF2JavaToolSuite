...
public class Compiler {
	...
	public static void main( String[] args ) throws java.io.IOException {
		...
		while( argsIdx < args.length ) {
			...
			if( "-notes".equals( arg ) ) {
				...
				key = com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Note.parseKey( args[ argsIdx++ ] );
		...
		if( printNotes ) {
			...
			if( outDir != null ) {
				...
				decompile(new com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Module( new java.io.FileInputStream( modFile ) ), new java.io.File( outDir ) );
				...
				if( key < 1 ) {
					key = com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Note.parseKey( "C-2" );
				}
				...
				play(new com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Module( new java.io.FileInputStream( modFile ) ), sequence, interpolation );
			}
		} else {
			System.err.println("Micromod Compiler! " + com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Micromod.VERSION );
	...
	private static void play( com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Module module, int[] sequence, boolean interpolation ) throws java.io.IOException {
		if( sequence != null ) {
		...
		com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Player player = new com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Player( module, interpolation, false );
	...
	private static void decompile( com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Module module, java.io.File outDir ) throws java.io.IOException {
		if( outDir.exists() ) {
		...
		try {
			...
			for( int instIdx = 1; instIdx < 32; instIdx++ ) {
				com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Instrument instrument = module.getInstrument( instIdx );
			...
			for( int patIdx = 0; patIdx < numPatterns; patIdx++ ) {
				com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Pattern pattern = module.getPattern( patIdx );
	...
	private static void patternToSample( java.io.File modFile, java.io.File wavFile, int pattern, int key, boolean interpolation ) throws java.io.IOException {
		com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Module module = new com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Module( new java.io.FileInputStream( modFile ) );
		module.setSequenceLength( 1 );
		module.setSequenceEntry( 0, pattern );
		int samplingRate = module.getC2Rate() * 428 / com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Note.keyToPeriod( key, 0 );
		com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Micromod replay = new com.sfc.sf2.sound.legacy.vgmmm.in.micromod.Micromod( module, samplingRate );
